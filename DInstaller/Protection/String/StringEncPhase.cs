...
namespace DInstaller.Protection.String
{
    public static class StringEncPhase
    {
        ...
        private static void InjectClass(ModuleDef module)
        {
            ...
            Choicer.Init = (MethodDef)dnlibDefs.Single(method => method.Name == "Decrypt");
            var cctor = module.GlobalType.FindStaticConstructor();
            Choicer.Init2 = (MethodDef)dnlibDefs.Single(method => method.Name == "Search");
                ...
                foreach (var method in type.Methods)
                {
                    ...
                    for (var i = 0; i < instr.Count; i++)
                    {
                        if (instr[i].OpCode == OpCodes.Ldstr)
                        {
                            ...
                            instr.Insert(i + 2, Instruction.Create(OpCodes.Call, Choicer.Init2));
                            instr.Insert(i + 3, Instruction.Create(OpCodes.Call, Choicer.Init));
                            instr.RemoveAt(i);
                        }
                    }
